...
class SendStoreIntegrationApprovalCallback implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        if ($merchant->integration_callback_status === 'delivered') {
            return;
        }

        $apiKey = $merchant->apiKeys
            ->first(fn ($key) => $key->is_active || $key->status === 'active');

        if (!$apiKey) {
            throw new RuntimeException('Active merchant API key not found.');
        }

        if (!$apiKey->api_secret_encrypted) {
            throw new RuntimeException('Encrypted merchant API secret is missing.');
        }

        $plainSecret = Crypt::decryptString($apiKey->api_secret_encrypted);
        $eventId = 'evt_' . Str::lower(Str::random(32));
        ...
        $payload = [
            ...
            'approved_services' => $merchant->approved_services ?? [],
            'pickup_location' => $pickupLocation
                ...
                : null,
            'approved_at' => $merchant->integration_approved_at?->toIso8601String(),
        ];
        ...
        try {
            $response = Http::withHeaders([
                ...
                ->timeout(20)
                ->post($merchant->integration_callback_url);

            if (!$response->successful()) {
                throw new RuntimeException(
                    'Store callback failed with HTTP ' . $response->status()
                );
            }

            $merchant->forceFill([
                'integration_callback_status' => 'delivered',
                'integration_callback_sent_at' => now(),
                'integration_callback_error' => null,
            ])->save();
        } catch (Throwable $exception) {
            $merchant->forceFill([
                'integration_callback_status' => 'failed',
                'integration_callback_error' => Str::limit(
