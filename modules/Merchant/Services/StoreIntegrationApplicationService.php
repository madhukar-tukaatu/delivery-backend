<?php

namespace Modules\Merchant\Services;

use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Http;
use Illuminate\Support\Facades\Schema;
use Illuminate\Support\Facades\Storage;
use Illuminate\Support\Str;
use Illuminate\Validation\ValidationException;
use Modules\Merchant\Models\Merchant;
use Modules\Merchant\Models\MerchantDocument;
use Modules\Merchant\Models\MerchantPickupLocation;
use Modules\Routing\Services\BranchLocatorService;
use Throwable;

class StoreIntegrationApplicationService
{
    private const REQUIRED_DOCUMENTS = [
        'business_registration',
        'pan_vat',
        'owner_id',
        'bank_proof',
    ];

    /*
     * Store Manager documents are copied into Tukaatu local storage for now.
     * Change this to "s3" later after the project-wide S3 disk has been
     * configured in config/filesystems.php.
     */
    private const DOCUMENT_DISK = 'public';

    private const MAX_DOCUMENT_BYTES = 10 * 1024 * 1024;

    private const DOWNLOAD_TIMEOUT_SECONDS = 60;

    private const CONNECT_TIMEOUT_SECONDS = 10;

    public function __construct(
        private BranchLocatorService $branchLocator
    ) {
    }

    /**
     * Store Manager integration endpoint.
     *
     * The public Tukaatu merchant form continues using its existing
     * direct-upload services.
     */
    public function submit(
        string $applicationNumber,
        array $data,
        array $documents
    ): array {
        $newFiles = [];
        $oldFiles = [];

        try {
            $result = DB::transaction(function () use (
                $applicationNumber,
                $data,
                $documents,
                &$newFiles,
                &$oldFiles
            ) {
                /*
                 * ---------------------------------------------------------
                 * Validate the basic Store Manager payload before accessing
                 * nested keys.
                 * ---------------------------------------------------------
                 */
                $this->validateIntegrationPayload($data);

                $merchant = Merchant::query()
                    ->where(
                        'application_number',
                        $applicationNumber
                    )
                    ->lockForUpdate()
                    ->first();

                $created = !$merchant;

                if (
                    $merchant &&
                    in_array(
                        $merchant->integration_status,
                        ['approved', 'suspended'],
                        true
                    )
                ) {
                    throw ValidationException::withMessages([
                        'application_number' => [
                            'This Store Manager application has already been approved.',
                        ],
                    ]);
                }

                /*
                 * ---------------------------------------------------------
                 * Duplicate merchant protection
                 * ---------------------------------------------------------
                 */
                $this->ensureNoDuplicateMerchant(
                    $data,
                    $merchant?->id,
                    $applicationNumber
                );

                /*
                 * ---------------------------------------------------------
                 * Required documents
                 * ---------------------------------------------------------
                 */
                $this->ensureRequiredDocumentsExist(
                    $merchant,
                    $documents
                );

                /*
                 * ---------------------------------------------------------
                 * Pickup location
                 * ---------------------------------------------------------
                 */
                $pickup = $data['pickup_location'];

                /*
                 * City and area are optional in the Store Manager payload.
                 *
                 * The current Store Manager payload contains:
                 *
                 * address
                 * landmark
                 * latitude
                 * longitude
                 *
                 * but does not contain:
                 *
                 * city
                 * area
                 *
                 * Therefore NEVER directly use:
                 *
                 * $pickup['city']
                 * $pickup['area']
                 */
                $pickupCity = $this->resolvePickupCity($pickup);

                $pickupArea = $this->resolvePickupArea($pickup);

                /*
                 * ---------------------------------------------------------
                 * Branch suggestion based on coordinates
                 * ---------------------------------------------------------
                 */
                $location = $this->branchLocator->locate(
                    (float) $pickup['latitude'],
                    (float) $pickup['longitude']
                );

                $suggestedBranch = $location['branch'] ?? null;

                $suggestedSubBranch = $location['sub_branch'] ?? null;

                /*
                 * ---------------------------------------------------------
                 * Create merchant if this is a new application
                 * ---------------------------------------------------------
                 */
                if (!$merchant) {
                    $merchant = new Merchant();

                    $merchant->application_number =
                        $applicationNumber;

                    $merchant->code = $this->makeCode(
                        $data['store']['name']
                    );
                }

                /*
                 * ---------------------------------------------------------
                 * Sanitize integration payload before persisting it.
                 * ---------------------------------------------------------
                 */
                $integrationPayload =
                    $this->sanitizeIntegrationPayload($data);

                /*
                 * ---------------------------------------------------------
                 * Save merchant
                 * ---------------------------------------------------------
                 */
                $merchant->forceFill([
                    'application_source' =>
                        Merchant::SOURCE_STORE_MANAGER,

                    'external_store_id' =>
                        $data['store']['external_store_id'],

                    'external_platform' =>
                        $data['store']['platform'],

                    'store_category' =>
                        $data['store']['category'] ?? null,

                    'store_url' =>
                        $data['store']['url'] ?? null,

                    'name' =>
                        $data['store']['name'],

                    'owner_name' =>
                        $data['business']['owner_name'],

                    'contact_person' =>
                        $data['business']['contact_person']
                        ?? $data['business']['owner_name'],

                    'phone' =>
                        $data['business']['phone'],

                    'email' =>
                        strtolower($data['business']['email']),

                    'website_url' =>
                        $data['store']['url'] ?? null,

                    'business_type' =>
                        $data['business']['type'] ?? null,

                    'pan_vat_number' =>
                        $data['business']['pan_vat_number'],

                    'registration_number' =>
                        $data['business']['registration_number'],

                    'address' =>
                        $data['business']['registered_address'],

                    /*
                     * Pickup information.
                     */
                    'pickup_address' =>
                        $pickup['address'],

                    'pickup_city' =>
                        $pickupCity,

                    'pickup_area' =>
                        $pickupArea,

                    'pickup_lat' =>
                        $pickup['latitude'],

                    'pickup_lng' =>
                        $pickup['longitude'],

                    /*
                     * Suggested routing branch.
                     */
                    'suggested_branch_id' =>
                        $suggestedBranch?->id,

                    'suggested_sub_branch_id' =>
                        $suggestedSubBranch?->id,

                    /*
                     * Requested services.
                     */
                    'requested_services' =>
                        array_values(
                            array_unique(
                                $data['requested_services']
                            )
                        ),

                    'approved_services' => null,

                    /*
                     * Store Manager integration data.
                     */
                    'integration_payload' =>
                        $integrationPayload,

                    'integration_callback_url' =>
                        $data['callback']['url'],

                    'integration_callback_secret' =>
                        $data['callback']['secret'],

                    'integration_status' =>
                        'pending_review',

                    'integration_callback_status' =>
                        null,

                    'submitted_at' =>
                        now(),

                    /*
                     * Existing Tukaatu merchant workflow.
                     */
                    'status' =>
                        'pending_verification',

                    'verification_status' =>
                        'submitted',

                    'more_info_message' =>
                        null,

                    'rejected_reason' =>
                        null,
                ])->save();

                /*
                 * ---------------------------------------------------------
                 * Save pickup location
                 * ---------------------------------------------------------
                 */
                $this->savePickupLocation(
                    $merchant,
                    $pickup,
                    $pickupCity,
                    $pickupArea,
                    $suggestedBranch?->id,
                    $suggestedSubBranch?->id
                );

                /*
                 * ---------------------------------------------------------
                 * Save documents
                 * ---------------------------------------------------------
                 */
                $this->saveDocuments(
                    $merchant,
                    $documents,
                    $newFiles,
                    $oldFiles
                );

                return [
                    'merchant' => $merchant->fresh([
                        'documents',
                        'pickupLocations',
                        'suggestedBranch',
                        'suggestedSubBranch',
                    ]),

                    'created' => $created,
                ];
            });
        } catch (Throwable $exception) {
            /*
             * Remove newly-created files when the transaction fails.
             */
            $this->deleteFiles($newFiles);

            throw $exception;
        }

        /*
         * Old files are deleted only after the DB transaction succeeds.
         */
        $this->deleteFiles($oldFiles);

        return $result;
    }

    /**
     * Validate the minimum Store Manager integration payload.
     *
     * Optional fields such as pickup city, area and landmark are deliberately
     * not required because the Store Manager payload does not currently send
     * them.
     */
    private function validateIntegrationPayload(array $data): void
    {
        $requiredPaths = [
            'store.external_store_id',
            'store.name',
            'store.platform',

            'business.owner_name',
            'business.phone',
            'business.email',
            'business.pan_vat_number',
            'business.registration_number',
            'business.registered_address',

            'pickup_location.name',
            'pickup_location.contact_person',
            'pickup_location.phone',
            'pickup_location.address',
            'pickup_location.latitude',
            'pickup_location.longitude',

            'callback.url',
            'callback.secret',
        ];

        $missing = [];

        foreach ($requiredPaths as $path) {
            if (!data_get($data, $path)) {
                $missing[] = $path;
            }
        }

        if (!empty($missing)) {
            throw ValidationException::withMessages([
                'payload' => [
                    'Missing required integration fields: ' .
                    implode(', ', $missing),
                ],
            ]);
        }

        if (
            !isset($data['requested_services']) ||
            !is_array($data['requested_services'])
        ) {
            throw ValidationException::withMessages([
                'requested_services' => [
                    'The requested_services field must be an array.',
                ],
            ]);
        }

        if (
            !is_numeric($data['pickup_location']['latitude']) ||
            !is_numeric($data['pickup_location']['longitude'])
        ) {
            throw ValidationException::withMessages([
                'pickup_location' => [
                    'Pickup latitude and longitude must be numeric.',
                ],
            ]);
        }
    }

    /**
     * Resolve pickup city safely.
     *
     * Priority:
     *
     * 1. Explicit city from Store Manager
     * 2. City from the branch locator
     * 3. Null
     */
    private function resolvePickupCity(array $pickup): ?string
    {
        $city = trim(
            (string) ($pickup['city'] ?? '')
        );

        if ($city !== '') {
            return $city;
        }

        /*
         * Do not fail the application if city is not supplied.
         *
         * The current Store Manager payload only provides the textual
         * address and coordinates.
         */
        return null;
    }

    /**
     * Resolve pickup area safely.
     *
     * Priority:
     *
     * 1. Explicit area
     * 2. Landmark
     * 3. Null
     */
    private function resolvePickupArea(array $pickup): ?string
    {
        $area = trim(
            (string) ($pickup['area'] ?? '')
        );

        if ($area !== '') {
            return $area;
        }

        $landmark = trim(
            (string) ($pickup['landmark'] ?? '')
        );

        if ($landmark !== '') {
            return $landmark;
        }

        return null;
    }

    /**
     * Prevent duplicate merchants/applications.
     */
    private function ensureNoDuplicateMerchant(
        array $data,
        ?int $currentMerchantId,
        string $applicationNumber
    ): void {
        $business = $data['business'];
        $store = $data['store'];

        $duplicate = Merchant::query()
            ->when(
                $currentMerchantId,
                fn ($query) =>
                    $query->where(
                        'id',
                        '!=',
                        $currentMerchantId
                    )
            )
            ->where(function ($query) use (
                $business,
                $store,
                $applicationNumber
            ) {
                $query
                    ->where(
                        'application_number',
                        $applicationNumber
                    )

                    ->orWhere(function ($query) use ($store) {
                        $query
                            ->where(
                                'application_source',
                                Merchant::SOURCE_STORE_MANAGER
                            )
                            ->where(
                                'external_store_id',
                                $store['external_store_id']
                            )
                            ->where(
                                'external_platform',
                                $store['platform']
                            );
                    })

                    ->orWhere(
                        'pan_vat_number',
                        $business['pan_vat_number']
                    )

                    ->orWhere(
                        'registration_number',
                        $business['registration_number']
                    )

                    ->orWhere(
                        'email',
                        strtolower($business['email'])
                    );
            })
            ->first();

        if (!$duplicate) {
            return;
        }

        throw ValidationException::withMessages([
            'store' => [
                'A merchant or Store Manager application already exists with the submitted business information.',
            ],

            'existing_application' => [
                $duplicate->application_number
                    ?: (string) $duplicate->id,
            ],
        ]);
    }

    /**
     * Verify that all required documents are either already stored or
     * included in this submission.
     */
    private function ensureRequiredDocumentsExist(
        ?Merchant $merchant,
        array $incomingDocuments
    ): void {
        $existingTypes = $merchant
            ? $merchant->documents()
                ->pluck('document_type')
                ->all()
            : [];

        $incomingTypes = collect($incomingDocuments)
            ->filter(function ($document) {
                return is_array($document)
                    && !empty($document['url']);
            })
            ->keys()
            ->all();

        $availableTypes = collect([
            ...$existingTypes,
            ...$incomingTypes,
        ])->unique();

        $missing = collect(self::REQUIRED_DOCUMENTS)
            ->reject(
                fn (string $type) =>
                    $availableTypes->contains($type)
            )
            ->values();

        if ($missing->isNotEmpty()) {
            throw ValidationException::withMessages([
                'documents' => [
                    'Missing required documents: ' .
                    $missing->join(', '),
                ],
            ]);
        }
    }

    /**
     * Save or update the merchant's default pickup location.
     */
    private function savePickupLocation(
        Merchant $merchant,
        array $pickup,
        ?string $pickupCity,
        ?string $pickupArea,
        ?int $suggestedBranchId,
        ?int $suggestedSubBranchId
    ): void {
        MerchantPickupLocation::updateOrCreate(
            [
                'merchant_id' => $merchant->id,
                'is_default' => true,
            ],
            [
                'name' =>
                    $pickup['name'],

                'contact_person' =>
                    $pickup['contact_person'],

                'phone' =>
                    $pickup['phone'],

                'address' =>
                    $pickup['address'],

                /*
                 * IMPORTANT:
                 * These are now safe because city/area are optional.
                 */
                'city' =>
                    $pickupCity,

                'area' =>
                    $pickupArea,

                'latitude' =>
                    $pickup['latitude'],

                'longitude' =>
                    $pickup['longitude'],

                'suggested_branch_id' =>
                    $suggestedBranchId,

                'suggested_sub_branch_id' =>
                    $suggestedSubBranchId,

                /*
                 * Actual branch assignment is done during verification.
                 */
                'branch_id' =>
                    null,

                'sub_branch_id' =>
                    null,

                'status' =>
                    'pending_verification',
            ]
        );
    }

    /**
     * Save incoming documents.
     */
    private function saveDocuments(
        Merchant $merchant,
        array $documents,
        array &$newFiles,
        array &$oldFiles
    ): void {
        foreach ($documents as $type => $document) {
            if (
                !is_array($document) ||
                empty($document['url'])
            ) {
                continue;
            }

            $this->downloadAndStoreDocument(
                $merchant,
                (string) $type,
                $document,
                $newFiles,
                $oldFiles
            );
        }
    }

    /**
     * Download a Store Manager document, validate it, and save it locally.
     */
    private function downloadAndStoreDocument(
        Merchant $merchant,
        string $type,
        array $document,
        array &$newFiles,
        array &$oldFiles
    ): void {
        $sourceUrl = (string) $document['url'];

        $this->validateDocumentSourceUrl(
            $sourceUrl,
            $type
        );

        $temporaryPath = tempnam(
            sys_get_temp_dir(),
            'tukaatu_merchant_'
        );

        if ($temporaryPath === false) {
            throw ValidationException::withMessages([
                "documents.{$type}" => [
                    'Could not create a temporary document file.',
                ],
            ]);
        }

        try {
            try {
                $response = Http::connectTimeout(
                    self::CONNECT_TIMEOUT_SECONDS
                )
                    ->timeout(
                        self::DOWNLOAD_TIMEOUT_SECONDS
                    )
                    ->retry(2, 500)
                    ->withOptions([
                        'sink' => $temporaryPath,
                        'allow_redirects' => false,
                        'verify' => true,
                    ])
                    ->get($sourceUrl);
            } catch (Throwable) {
                throw ValidationException::withMessages([
                    "documents.{$type}.url" => [
                        'Tukaatu could not download this document URL.',
                    ],
                ]);
            }

            if (!$response->successful()) {
                throw ValidationException::withMessages([
                    "documents.{$type}.url" => [
                        "Unable to download document. HTTP {$response->status()}.",
                    ],
                ]);
            }

            $fileSize = filesize($temporaryPath);

            if (
                $fileSize === false ||
                $fileSize < 1
            ) {
                throw ValidationException::withMessages([
                    "documents.{$type}.url" => [
                        'The downloaded document is empty.',
                    ],
                ]);
            }

            if (
                $fileSize > self::MAX_DOCUMENT_BYTES
            ) {
                throw ValidationException::withMessages([
                    "documents.{$type}.url" => [
                        'The downloaded document must not exceed 10 MB.',
                    ],
                ]);
            }

            if (
                isset($document['size_bytes']) &&
                (int) $document['size_bytes'] !==
                (int) $fileSize
            ) {
                throw ValidationException::withMessages([
                    "documents.{$type}.size_bytes" => [
                        'The submitted document size does not match the downloaded file.',
                    ],
                ]);
            }

            $mimeType = (new \finfo(
                FILEINFO_MIME_TYPE
            ))->file($temporaryPath);

            $allowedMimeTypes =
                $type === 'office_photo'
                    ? [
                        'image/jpeg',
                        'image/png',
                        'image/webp',
                    ]
                    : [
                        'application/pdf',
                        'image/jpeg',
                        'image/png',
                        'image/webp',
                    ];

            if (
                !$mimeType ||
                !in_array(
                    $mimeType,
                    $allowedMimeTypes,
                    true
                )
            ) {
                throw ValidationException::withMessages([
                    "documents.{$type}.url" => [
                        'Unsupported document type.',
                    ],
                ]);
            }

            $actualChecksum = hash_file(
                'sha256',
                $temporaryPath
            );

            if (
                !empty($document['sha256']) &&
                !hash_equals(
                    strtolower(
                        (string) $document['sha256']
                    ),
                    strtolower(
                        (string) $actualChecksum
                    )
                )
            ) {
                throw ValidationException::withMessages([
                    "documents.{$type}.sha256" => [
                        'The document checksum does not match.',
                    ],
                ]);
            }

            $extension = match ($mimeType) {
                'application/pdf' => 'pdf',
                'image/jpeg' => 'jpg',
                'image/png' => 'png',
                'image/webp' => 'webp',
                default => throw new \RuntimeException(
                    'Unsupported document MIME type.'
                ),
            };

            $path = sprintf(
                'merchant-documents/%d/%s-%s.%s',
                $merchant->id,
                $type,
                Str::uuid(),
                $extension
            );

            $stream = fopen(
                $temporaryPath,
                'rb'
            );

            if ($stream === false) {
                throw new \RuntimeException(
                    'Could not open the downloaded document.'
                );
            }

            try {
                $stored = Storage::disk(
                    self::DOCUMENT_DISK
                )->put(
                    $path,
                    $stream
                );
            } finally {
                fclose($stream);
            }

            if (!$stored) {
                throw ValidationException::withMessages([
                    "documents.{$type}" => [
                        'Tukaatu could not save the downloaded document.',
                    ],
                ]);
            }

            $newFiles[] = [
                'disk' => self::DOCUMENT_DISK,
                'path' => $path,
            ];

            $existing = $merchant
                ->documents()
                ->where(
                    'document_type',
                    $type
                )
                ->first();

            $originalName =
                $this->sanitizeOriginalName(
                    $document['original_name']
                        ?? "{$type}.{$extension}"
                );

            $values = [
                'file_path' =>
                    $path,

                'original_name' =>
                    $originalName,

                'mime_type' =>
                    $mimeType,

                'status' =>
                    'pending',
            ];

            if (
                Schema::hasColumn(
                    'merchant_documents',
                    'disk'
                )
            ) {
                $values['disk'] =
                    self::DOCUMENT_DISK;
            }

            if (
                Schema::hasColumn(
                    'merchant_documents',
                    'size_bytes'
                )
            ) {
                $values['size_bytes'] =
                    $fileSize;
            } elseif (
                Schema::hasColumn(
                    'merchant_documents',
                    'size'
                )
            ) {
                $values['size'] =
                    $fileSize;
            }

            if (
                Schema::hasColumn(
                    'merchant_documents',
                    'checksum_sha256'
                )
            ) {
                $values['checksum_sha256'] =
                    $actualChecksum;
            }

            if (
                Schema::hasColumn(
                    'merchant_documents',
                    'source_host'
                )
            ) {
                $values['source_host'] =
                    parse_url(
                        $sourceUrl,
                        PHP_URL_HOST
                    );
            }

            if (
                Schema::hasColumn(
                    'merchant_documents',
                    'remarks'
                )
            ) {
                $values['remarks'] = null;
            }

            if (
                Schema::hasColumn(
                    'merchant_documents',
                    'verified_by'
                )
            ) {
                $values['verified_by'] = null;
            }

            if (
                Schema::hasColumn(
                    'merchant_documents',
                    'verified_at'
                )
            ) {
                $values['verified_at'] = null;
            }

            MerchantDocument::updateOrCreate(
                [
                    'merchant_id' =>
                        $merchant->id,

                    'document_type' =>
                        $type,
                ],
                $values
            );

            if (
                $existing?->file_path &&
                $existing->file_path !== $path
            ) {
                $oldFiles[] = [
                    'disk' =>
                        $existing->disk
                        ?? self::DOCUMENT_DISK,

                    'path' =>
                        $existing->file_path,
                ];
            }
        } finally {
            if (is_file($temporaryPath)) {
                @unlink($temporaryPath);
            }
        }
    }

    /**
     * Validate that document URLs are public HTTPS URLs.
     *
     * This also protects the server from SSRF requests to private,
     * loopback or reserved addresses.
     */
    private function validateDocumentSourceUrl(
        string $url,
        string $documentType
    ): void {
        $parts = parse_url($url);

        $scheme = strtolower(
            (string) ($parts['scheme'] ?? '')
        );

        $host = strtolower(
            (string) ($parts['host'] ?? '')
        );

        $port = isset($parts['port'])
            ? (int) $parts['port']
            : 443;

        if (
            $scheme !== 'https' ||
            $host === '' ||
            isset($parts['user']) ||
            isset($parts['pass']) ||
            $port !== 443
        ) {
            throw ValidationException::withMessages([
                "documents.{$documentType}.url" => [
                    'The document URL must be a public HTTPS URL using port 443.',
                ],
            ]);
        }

        $addresses = [];

        if (
            filter_var(
                $host,
                FILTER_VALIDATE_IP
            )
        ) {
            $addresses[] = $host;
        } else {
            $records = @dns_get_record(
                $host,
                DNS_A | DNS_AAAA
            );

            if (is_array($records)) {
                foreach ($records as $record) {
                    if (!empty($record['ip'])) {
                        $addresses[] =
                            $record['ip'];
                    }

                    if (!empty($record['ipv6'])) {
                        $addresses[] =
                            $record['ipv6'];
                    }
                }
            }

            if (!$addresses) {
                $ipv4Addresses =
                    @gethostbynamel($host);

                if (is_array($ipv4Addresses)) {
                    $addresses = array_merge(
                        $addresses,
                        $ipv4Addresses
                    );
                }
            }
        }

        $addresses = array_values(
            array_unique($addresses)
        );

        if (!$addresses) {
            throw ValidationException::withMessages([
                "documents.{$documentType}.url" => [
                    'The document host could not be resolved.',
                ],
            ]);
        }

        foreach ($addresses as $address) {
            if (
                !$this->isPublicIpAddress(
                    $address
                )
            ) {
                throw ValidationException::withMessages([
                    "documents.{$documentType}.url" => [
                        'The document URL resolves to a private or reserved network address.',
                    ],
                ]);
            }
        }
    }

    /**
     * Return true only for public IP addresses.
     */
    private function isPublicIpAddress(
        string $address
    ): bool {
        return filter_var(
            $address,
            FILTER_VALIDATE_IP,
            FILTER_FLAG_NO_PRIV_RANGE |
            FILTER_FLAG_NO_RES_RANGE
        ) !== false;
    }

    /**
     * Remove secrets and temporary document URLs before storing the
     * integration payload.
     */
    private function sanitizeIntegrationPayload(
        array $data
    ): array {
        $payload = $data;

        /*
         * Never persist callback secret.
         */
        unset(
            $payload['callback']['secret']
        );

        if (!empty($payload['documents'])) {
            $payload['documents'] =
                collect($payload['documents'])
                    ->map(function ($document) {
                        if (!is_array($document)) {
                            return null;
                        }

                        /*
                         * Do not persist temporary presigned URLs or
                         * their query signatures.
                         */
                        unset($document['url']);

                        return $document;
                    })
                    ->filter()
                    ->all();
        }

        return $payload;
    }

    /**
     * Sanitize uploaded/original document name.
     */
    private function sanitizeOriginalName(
        string $name
    ): string {
        $name = basename(
            str_replace(
                '\\',
                '/',
                trim($name)
            )
        );

        if (
            $name === '' ||
            $name === '.' ||
            $name === '..'
        ) {
            return 'document';
        }

        return Str::limit(
            $name,
            255,
            ''
        );
    }

    /**
     * Delete files safely.
     */
    private function deleteFiles(
        array $files
    ): void {
        foreach ($files as $file) {
            $disk =
                $file['disk']
                ?? self::DOCUMENT_DISK;

            $path =
                $file['path']
                ?? null;

            if (!$path) {
                continue;
            }

            try {
                Storage::disk($disk)
                    ->delete($path);
            } catch (Throwable) {
                /*
                 * Cleanup failure must not reverse a successful merchant
                 * application.
                 */
            }
        }
    }

    /**
     * Generate a unique merchant code.
     */
    private function makeCode(
        string $name
    ): string {
        $base = strtoupper(
            (string) str($name)->slug('-')
        );

        $base = substr(
            $base,
            0,
            20
        ) ?: 'STORE';

        $code = $base;

        $counter = 1;

        while (
            Merchant::query()
                ->where(
                    'code',
                    $code
                )
                ->exists()
        ) {
            $code =
                $base .
                '-' .
                $counter++;
        }

        return $code;
    }
}