...
final class AdminBranchTransferRouteController extends Controller
{
    ...
    /**
     * Preview / validate / calculate transfer route.
     */
    public function preview(Request $request): JsonResponse
    {
        ...
        $originBranchId = (int) $validated[
            'origin_branch_id'
        ];

        $destinationBranchId = (int) $validated[
            'destination_branch_id'
        ];
        ...
        if (
            ...
            throw ValidationException::withMessages([
                'transit_branch_ids' => [
                    'Origin branch cannot be used as a transit branch.',
                ],
            ]);
        }

        /*
         * Destination cannot be transit.
         */
        if (
            in_array(
                $destinationBranchId,
            ...
            throw ValidationException::withMessages([
                'transit_branch_ids' => [
                    'Destination branch cannot be used as a transit branch.',
                ],
            ]);
        }
        ...
        /*
         * Resolve every direct transfer lane.
         */
        $laneMappings = [];

        for (
            $index = 0;
            $index < count($branchPath) - 1;
            $index++
        ) {
         ...
         * IMPORTANT:
         *
         * This calls ConfiguredTransferRouteService.
         *
         * That service now reads:
         *
         * branch_route_rates.pickup_branch_id
         * branch_route_rates.delivery_branch_id
         * branch_route_rates.base_rate
         * branch_route_rates.is_active
         */
        $calculation = $this->resolver
            ->validateAndCalculateLanes(
                $laneMappings,
                $originBranchId,
                $destinationBranchId,
                $serviceType
        ...
        return response()->json([
            ...
            'message' =>
                'Transfer route validated successfully.',
            ...
            'data' => [
                ...
                'base_rate' =>
                    $calculation['calculated_base_rate'],
                ...
                'lane_count' =>
                    $calculation['lane_count'],

                'transfer_count' =>
                    $calculation['transfer_count'],

                'transit_count' =>
                    $calculation['transit_count'],

                'total_distance_km' =>
                    $calculation['total_distance_km'],

                'total_estimated_hours' =>
                    $calculation['total_estimated_hours'],

                'path' =>
                    $calculation['path'],

                'path_text' =>
                    $calculation['path_text'],

                'transit_branches' =>
                    $calculation['transit_branches'],
    ...
    public function store(
        ...
        /*
         * Calculate from actual Branch Pricing.
         */
        $calculation = $this->calculateFromRequest(
            $validated
        );

        /*
         * NEVER trust frontend base_rate.
         */
        $validated['base_rate'] =
            $calculation['calculated_base_rate'];

        $validated['route_type'] = 'transfer';
    ...
    public function update(
        ...
        /*
         * Always use current Branch Pricing.
         */
        $validated['base_rate'] =
            $calculation['calculated_base_rate'];
        ...
        return response()->json([
            ...
            'message' =>
                'Transfer route updated successfully.',
    ...
    public function updateStatus(
        ...
        return response()->json([
            ...
            'message' =>
                $transferRoute->is_active
                    ? 'Transfer route activated successfully.'
                    : 'Transfer route disabled successfully.',

            'data' => [
                'id' =>
                    (int) $transferRoute->id,

                'is_active' =>
                    (bool) $transferRoute->is_active,
            ],
        ]);
    }

    /**
     * Disable transfer route.
     */
    ...
    /**
     * Calculate pricing/lanes from create/update request.
     */
    private function calculateFromRequest(
        array $data
    ): array {
        $originBranchId = (int) (
            $data['origin_branch_id'] ?? 0
        );

        $destinationBranchId = (int) (
            $data['destination_branch_id'] ?? 0
        );
        ...
        if (
            ...
            throw ValidationException::withMessages([
                'transfer_route' => [
                    'Valid origin and destination branches are required.',
                ],
            ]);
        }
    ...
    private function formatRoute(
        ...
        foreach (
            ...
            if ($index === 0) {
                $path[] = [
                    ...
                    'name' =>
                        $lane->fromBranch?->name,

                    'code' =>
                        $lane->fromBranch?->code,
                ];
            }
        ...
        return [
            ...
            'origin_branch' => [
                'id' =>
                    (int) $route->origin_branch_id,

                'name' =>
                    $route->originBranch?->name,

                'code' =>
                    $route->originBranch?->code,
            ],

            'destination_branch' => [
                'id' =>
                    (int) $route->destination_branch_id,

                'name' =>
                    $route->destinationBranch?->name,

                'code' =>
                    $route->destinationBranch?->code,
            ],
