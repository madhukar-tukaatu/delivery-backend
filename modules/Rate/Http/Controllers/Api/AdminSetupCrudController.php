...
class AdminSetupCrudController extends Controller
{
    ...
    public function transferLanes(Request $r) { $q=DB::table('branch_transfer_lanes')->leftJoin('branches as fb','fb.id','=','branch_transfer_lanes.from_branch_id')->leftJoin('branches as tb','tb.id','=','branch_transfer_lanes.to_branch_id')->select('branch_transfer_lanes.*','fb.name as from_branch_name','tb.name as to_branch_name')->orderByDesc('branch_transfer_lanes.id'); return response()->json(['success'=>true,'data'=>$q->paginate($r->integer('per_page',25))]); }
