...
class MultiStorePricingServicePacketsPerProducts
{
    ...
    public function calculateOnly(
        ...
        return $this->formatCalculationResponse(
            $this->calculateStores($validated, $marketplaceId)
        );
    }
    ...
    public function calculateAndStore(
        ...
        return DB::transaction(function () use ($validated, $marketplaceId): array {
            $calculation = $this->calculateStores($validated, $marketplaceId);

            $checkoutQuoteNumber = $this->uniqueQuoteNumber('checkout_quotes', 'CQ');

            $checkoutQuoteId = DB::table('checkout_quotes')->insertGetId(
                $this->onlyExistingColumns('checkout_quotes', [
                    'quote_number'         => $checkoutQuoteNumber,
                    'external_checkout_id' => $validated['external_checkout_id'] ?? null,
                    'merchant_id'          => null,
                    ...
                    'service_type_id'      => $calculation['default_service_type_id'],
                    ...
                    'status'               => 'pending',
                    'expires_at'           => $this->toDatabaseDateTime($calculation['valid_until']),
                    'snapshot_json'        => $this->encodeJson($this->serialiseDates($calculation)),
                    'created_at'           => now(),
                    'updated_at'           => now(),
                ])
            );
    ...
    private function calculateStores(array $validated, int $marketplaceId): array
    {
        $stores = $validated['stores'] ?? [];

        if (!is_array($stores) || count($stores) === 0) {
            throw ValidationException::withMessages([
                'stores' => ['At least one marketplace store is required.'],
            ]);
        }
        ...
        $defaultServiceType   = (string) ($validated['service_type'] ?? 'standard');
        $defaultPaymentType   = (string) ($validated['payment_type'] ?? 'prepaid');
        $storeCalculations    = [];
        $productsTotal        = 0.0;
        $podTotal             = 0.0;
        $deliveryTotal        = 0.0;
        ...
        foreach ($stores as $index => $store) {
            if (!is_array($store)) {
                throw ValidationException::withMessages([
                    "stores.{$index}" => ['Each marketplace store must be an object.'],
                ]);
            }

            $summary     = $this->storeSummary($store, $index);
            $paymentType = (string) ($store['payment_type'] ?? $defaultPaymentType);
            $serviceType = (string) ($store['service_type'] ?? $defaultServiceType);
            $podAmount   = $paymentType === 'pod'
                ? (float) ($store['pod_amount'] ?? $summary['parcel_value'])
                : 0.0;

            $payload = [
                'store_id'           => isset($store['store_id']) ? (int) $store['store_id'] : null,
            ...
            try {
                $quote = $this->pricingEngine->calculate($payload, null);
            ...
            /*
             * Resolve the physical transfer route from the branch_route_rates
             * row that was matched during pricing. No lane resolver needed.
             */
            $transferRoute = null;
            $routeRateId   = data_get($quote, 'route.route_rate_id');

            if ($routeRateId) {
                $linkedRouteId = DB::table('branch_route_rates')
                    ->where('id', $routeRateId)
                    ->value('branch_transfer_route_id');

                if ($linkedRouteId) {
                    $linkedRoute = BranchTransferRoute::find((int) $linkedRouteId);

                    if ($linkedRoute) {
                        $transferRoute = $this->transferRouteResolver->formatRoute($linkedRoute);
                    }
                }
            }

            $pricingEstimatedHours     = (int) ($quote['estimated_hours'] ?? 0);
            $operationalEstimatedHours = (int) ($transferRoute['total_estimated_hours'] ?? 0);
            $effectiveEstimatedHours   = max($pricingEstimatedHours, $operationalEstimatedHours);

            $quote['pricing_estimated_hours']     = $pricingEstimatedHours;
            $quote['operational_estimated_hours'] = $operationalEstimatedHours;
            $quote['estimated_hours']             = $effectiveEstimatedHours;
            $quote['sla_due_at']                  = now()->addHours($effectiveEstimatedHours);
            $quote['transfer_route']              = $transferRoute;

            $defaultServiceTypeId ??= (int) $quote['service_type']['id'];
            $currency               = (string) ($quote['currency'] ?? $currency);
            $validUntil             = Carbon::parse($quote['valid_until']);

            if ($earliestValidUntil === null || $validUntil->lt($earliestValidUntil)) {
                $earliestValidUntil = $validUntil;
            }

            $checkoutEstimatedHours = max($checkoutEstimatedHours, $effectiveEstimatedHours);
            ...
            $storeCalculations[] = [
                ...
                'packet_count'           => (int) ($quote['packet_count'] ?? $summary['packet_count']),
                'parcel_weight'          => $summary['parcel_weight'],
                'parcel_value'           => $summary['parcel_value'],
                'parcel_type'            => $summary['parcel_type'],
                'payment_type'           => $paymentType,
        ...
        if ($earliestValidUntil === null || $defaultServiceTypeId === null) {
            throw ValidationException::withMessages([
                'pricing' => ['No valid marketplace pricing result was generated.'],
            ]);
        }
    ...
    {
        $products = is_array($store['products'] ?? null) ? array_values($store['products']) : [];
        $packets  = is_array($store['packets'] ?? null) ? array_values($store['packets']) : [];

        if ($products !== [] && $packets !== []) {
            throw ValidationException::withMessages([
                "stores.{$index}" => ['Products and packets cannot be used together for one store.'],
            ]);
        }

        $inputMode = (string) ($store['input_mode'] ?? match (true) {
            $products !== [] => 'products',
            $packets  !== [] => 'packets',
        ...
        $packetCount     = 0;
        $productCount    = 0;
        $containsFragile = ($store['parcel_type'] ?? null) === 'fragile';

        if ($products !== []) {
            foreach ($products as $key => $product) {
                $quantity  = max(1, (int) ($product['quantity'] ?? 1));
                $unitWeight = max(0, (float) ($product['unit_weight'] ?? 0));
                $unitPrice  = max(0, (float) ($product['unit_price'] ?? 0));
                $type       = $this->normaliseParcelType($product['parcel_type'] ?? 'non_fragile');
            ...
            foreach ($packets as $key => $packet) {
                $actualWeight  = max(0, (float) ($packet['actual_weight'] ?? 0));
                $declaredValue = max(0, (float) ($packet['declared_value'] ?? 0));
                $type          = $this->normaliseParcelType($packet['parcel_type'] ?? 'non_fragile');
            ...
            $parcelWeight = max(0, (float) ($store['parcel_weight'] ?? 0));
            $parcelValue  = max(0, (float) ($store['parcel_value'] ?? 0));
            $packetCount  = max(1, (int) ($store['packet_count'] ?? 1));
            $productCount = $packetCount;
        }

        if ($parcelWeight <= 0) {
            throw ValidationException::withMessages([
                "stores.{$index}.parcel_weight" => ['The calculated parcel weight must be greater than zero.'],
            ]);
        }

        $parcelType = $containsFragile ? 'fragile' : 'non_fragile';
    ...
    {
        foreach (['pickup_branch.id', 'delivery_branch.id', 'route', 'service_type.id', 'service_type.code', 'breakdown', 'final_price', 'currency', 'valid_until'] as $path) {
            if (data_get($quote, $path) === null) {
                throw ValidationException::withMessages([
                    "stores.{$storeIndex}.pricing" => ["Pricing engine response is missing {$path}."],
                ]);
            }
        }
        ...
        if ((float) $quote['final_price'] < 0) {
            throw ValidationException::withMessages([
                "stores.{$storeIndex}.pricing" => ['Calculated delivery price cannot be negative.'],
            ]);
        }
    }
    ...
    private function formatCalculationResponse(array $calculation): array
    {
        return [
            ...
            'store_quotes'    => array_map(
                fn (array $store): array => $this->formatStoreCalculation($store),
                $calculation['stores']
            ),
            'quote_stored'     => false,
            'shipment_created' => false,
    ...
    private function formatStoreCalculation(array $store): array
    {
        ...
        return [
            ...
            'transfer_stop_count'         => (int) ($store['transfer_route']['transit_count'] ?? 0),
            ...
            'vat'                         => $quote['vat'] ?? null,
            'pricing_estimated_hours'     => (int) ($quote['pricing_estimated_hours'] ?? 0),
            'operational_estimated_hours' => (int) ($quote['operational_estimated_hours'] ?? 0),
            'estimated_hours'             => (int) ($quote['estimated_hours'] ?? 0),
    ...
    private function saveStoreQuote(
        ...
        $storeQuoteNumber = $this->uniqueQuoteNumber('pricing_quotes', 'QT');

        $pricingQuoteId = DB::table('pricing_quotes')->insertGetId(
            $this->onlyExistingColumns('pricing_quotes', [
                'checkout_quote_id'  => $checkoutQuoteId,
                'quote_number'       => $storeQuoteNumber,
                'merchant_id'        => null,
                'marketplace_id'     => $marketplaceId,
                'store_id'           => $storeCalculation['store_id'],
                'external_store_id'  => $storeCalculation['external_store_id'],
                'pickup_branch_id'   => (int) $quote['pickup_branch']['id'],
                'delivery_branch_id' => (int) $quote['delivery_branch']['id'],
                ...
                'delivery_address'   => $checkoutCalculation['delivery']['address'],
                'delivery_latitude'  => $checkoutCalculation['delivery']['latitude'],
                'delivery_longitude' => $checkoutCalculation['delivery']['longitude'],
                ...
                'service_type_id'    => (int) $quote['service_type']['id'],
                'final_price'        => (float) $quote['final_price'],
                'currency'           => $quote['currency'] ?? 'NPR',
                'estimated_hours'    => isset($quote['estimated_hours']) ? (int) $quote['estimated_hours'] : null,
                'sla_due_at'         => $this->toDatabaseDateTime($quote['sla_due_at'] ?? null),
                'expires_at'         => $this->toDatabaseDateTime($quote['valid_until'] ?? null),
                'snapshot_json'      => $this->encodeJson($this->serialiseDates($quote)),
                'status'             => 'pending',
                'created_at'         => now(),
                'updated_at'         => now(),
            ])
        ...
        $this->saveQuoteItems((int) $pricingQuoteId, $storeCalculation);
    ...
    private function saveQuoteItems(int $pricingQuoteId, array $storeCalculation): void
    {
        ...
        foreach ($storeCalculation['products'] as $product) {
            ...
            $rows[] = $this->onlyExistingColumns('pricing_quote_items', [
        ...
        foreach ($storeCalculation['packets_input'] as $packet) {
            $rows[] = $this->onlyExistingColumns('pricing_quote_items', [
                ...
                'quantity'         => 1,
                'unit_weight'      => (float) ($packet['actual_weight'] ?? 0),
                'total_weight'     => (float) ($packet['actual_weight'] ?? 0),
                'unit_price'       => (float) ($packet['declared_value'] ?? 0),
                'total_price'      => (float) ($packet['declared_value'] ?? 0),
                'parcel_type'      => $effectiveParcelType,
                'created_at'       => now(),
                'updated_at'       => now(),
            ]);
    ...
    private function normaliseParcelType(mixed $value): string
    {
        return match (strtolower(trim((string) $value))) {
            'fragile'                                    => 'fragile',
            'non-fragile', 'non fragile', 'normal', 'regular' => 'non_fragile',
            default                                      => 'non_fragile',
        };
    }
    ...
    private function uniqueQuoteNumber(string $table, string $prefix): string
    {
        do {
            $quoteNumber = sprintf('%s-%s-%s', $prefix, now()->format('YmdHisv'), Str::upper(Str::random(8)));
            $exists      = DB::table($table)->where('quote_number', $quoteNumber)->exists();
        } while ($exists);
    ...
    private function encodeJson(array $value): string
    {
        try {
            ...
            throw ValidationException::withMessages([
                'pricing' => ['The pricing snapshot could not be encoded.'],
            ]);
        }
    }
