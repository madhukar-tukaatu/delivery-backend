...
class ShipmentWorkflowService
{
    ...
    public function createWorkflow(object $shipment): void
    {
        ...
        /*
    |--------------------------------------------------------------------------
    | Create ONLY pickup task
    |--------------------------------------------------------------------------
    */

        $pickupTaskId = $this->createTask(
            $shipment->id,
            $shipment->origin_sub_branch_id
                ?: $shipment->origin_branch_id,
            'pickup',
            $priority,
            'Pickup pending from customer or merchant.'
        );
        ...
        $this->notify(
            ...
            $pickupTaskId,
            'New pickup pending',
            'A new pickup task has been created.',
            'pickup_pending'
        );
    }
